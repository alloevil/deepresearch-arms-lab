"""Arm C4：C3 + 深度写作（单变量迭代：只改写作阶段的提示词）。

动机：round2 典型题上所有系统的最低维度都是 depth（C3 7.5, B 8.0）。
改法：节写作提示加入「深度四要素」硬性要求（数据点/多方对比/机制解释/批判性评估），
结论提示加入不确定性与反方观点要求。研究阶段与 C3 完全一致。
"""
import json
import re
import sys
...
def write_report(state: ResearchState, logger: TraceLogger) -> str:
    ...
    for sec in state.sections:
        ...
        prompt = (
            ...
            f"撰写报告的「{sec['title']}」一节（700-1000字）。规则：\n"
            f"1. 只使用下面编号证据中的信息，行文中用 [E编号] 标注依据；\n"
            f"2. 证据不足的方面明确写“现有资料未覆盖”，不要编造；\n"
            f"3. 【深度四要素，缺一不可】a) 引用证据中的具体数据点（数字/日期/金额）；"
            f"b) 多方对比（不同主体、口径或立场并列比较）；"
            f"c) 机制解释（为什么——给出因果链条，而非罗列现象）；"
            f"d) 批判性评估（指出证据的局限、口径差异或与其他证据的矛盾）。\n"
            f"直接输出正文，不要输出节标题。\n\n证据：\n{ev_text[:20000]}")
    ...
    conclusion = _write_with_continuation(
        ...
        f"3. 观点明确，不要复述各节内容；\n"
        f"4. 必须包含：结论成立的关键假设、主要不确定性、以及最有力的反方论点。\n"
        f"直接输出正文。\n\n草稿：\n{draft[:30000]}",
        logger, "write:结论")
    parts.append(f"## 结论\n\n{conclusion}")
...
if __name__ == "__main__":
    logger = TraceLogger("/tmp/armC4_smoke.jsonl")
