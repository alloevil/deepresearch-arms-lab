"""Arm F5 (Hybrid-v5)：F4 + 信源分级 / 深度要素机械检 / 冗长治理 / rubric 动态生长。

相对 arm_f4_scaffold 的改动（F4 裁判 rationale 残余失分点 → 修法）：
1. 信源权威性分级（q09"部分信源依赖博客"）：verify extended 检查低档信源占比
   与关键节的权威信源覆盖；协议加信源分级纪律
2. 深度四要素机械检测（depth 8.5 天花板）：四要素从提示词约束升级为可核对项
   （数字密度/对比标记/因果连接/批判用语），"LLM 做抽取、代码做判断"延伸到深度
3. 冗长治理（q05"部分段落略显冗长"）：超长低密度节机械报警；协议加篇幅纪律
4. rubric 动态生长：agent 可把研究中发现的新验收项追加进 rubric.json 的
   discovered 数组；门禁验证取"初始 rubric ∪ agent 追加项"（只增不减）
"""
import json
import os
import re
...
- python3 {vcli} check {out} --question question.txt --rubric rubric.json --extended ——
...
if __name__ == "__main__":
    import tempfile
    wd = Path(tempfile.mkdtemp(prefix="armF5_"))
    logger = TraceLogger("/tmp/armF5_smoke.jsonl")
