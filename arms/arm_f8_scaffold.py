"""Arm F8 (Hybrid-v8)：F6 + 引用核验缺口反馈（单变量迭代）。

相对 arm_f6_scaffold 的唯一机制改动（借 FineVerify"验证结果要反哺生成"的
思想缺口，见 survey 笔记方向一）：
- F6 的深检抽查"论断-引用"对，发现不支撑时失败信息只有 URL，
  修订 agent 不知道该验证什么论断；
- F8 的失败信息携带论断原文（verify_cli 已升级），_search_hints 为每条
  不支撑论断生成针对性验证搜索建议——把"验证发现的缺口"变成
  "下一轮检索的输入"。

另：本版起 common.read_page 具备 browser-use 渲染降级（JS 页面），
属公共基础设施，对所有 arm 生效，不计入 F8 的单变量。
"""
import json
import os
import re
...
def _search_hints(retrieval_fails: list[str]) -> str:
    ...
    for f in retrieval_fails:
        if "不支撑论断" in f:
            # F8 缺口反馈：为每条不支撑论断生成验证式搜索建议
            for claim in re.findall(r"不支撑论断：「(.+?)」", f):
                kw = re.sub(r"[，。；、\s]+", " ", claim)[:30].strip()
                hints.append(f'- 验证论断「{claim[:40]}…」：search "{kw}"，'
                             f'无果则提取其中的实体/数字改用英文再搜')
            continue
        m = re.search(r"「(.+?)」", f)
        if m:
            ent = m.group(1)
...
if __name__ == "__main__":
    import tempfile
    wd = Path(tempfile.mkdtemp(prefix="armF8_"))
    logger = TraceLogger("/tmp/armF8_smoke.jsonl")
