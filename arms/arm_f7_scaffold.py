"""Arm F7 (Hybrid-v7)：F6 + 任务类型条件化协议（type-conditioned protocol）。

动机：评估型任务（q09 类）是所有版本的系统性短板（8.5 vs 其余 9.0+），
one-size-fits-all 协议对"最终交付判断"的任务缺少结构要求。

相对 arm_f6_scaffold 的改动（单变量 = 类型条件化这一个概念）：
1. extract_rubric 增判 task_type（enumerative/comparative/evaluative/exploratory）
2. 协议按类型追加专属条款（评估型:条件化判断/置信依据/最强反方/先行指标；
   对比型:统一维度对比表；枚举型:同构信息、宁缺勿凑）
3. 验证联动:check_type_requirements 机械核对类型条款（骑墙检测/反方论点检测/
   对比结构检测）——协议要求什么验证器就查什么,不留君子协定
"""
import json
import os
import re
...
TYPE_PROTOCOLS = {
    "evaluative": """
8. 【评估型任务专属】最终章节必须是"判断"而非"总结"：
   a) 明确的条件化结论（"若 X 持续，则 Y；反之则 Z"），禁止"机遇与挑战并存"式骑墙；
   b) 给出判断的置信依据（哪些证据支撑、权重如何）；
   c) 呈现最强反方论点并正面回应，而非忽略；
   d) 列出 2-3 个可观察的先行指标（什么信号出现说明判断需要修正）。""",
    "comparative": """
8. 【对比型任务专属】对比必须结构化：给出统一维度的对比表（各主体同维度并列），
   差异要有机制解释（为何不同），禁止各写一段互不对话。""",
    "enumerative": """
8. 【枚举型任务专属】数量要求逐项达标后，为每个实例提供同构信息（相同字段），
   保证可比性；宁缺勿凑（无实质信息的凑数实例不合格）。""",
    "exploratory": "",
}
...
- python3 {vcli} check {out} --question question.txt --rubric rubric.json --extended --actions --task-type <rubric.json 中的 task_type> ——
...
6. 【内省自检】把报告写入 {out} 后，必须运行上面的质量自检命令，若返回
   ...
   或已尽力（最多自检 3 次）。
7. 重要论断标注来源 URL；报告用 Markdown 分节，含"参考来源"一节。{type_protocol}
...
def _verify_plus(out_file: Path, question: str, workdir: Path,
                 deep: bool = False, rubric: dict | None = None,
                 task_type: str | None = None) -> dict:
    result = verify(str(out_file), question, deep=deep, rubric=rubric,
                    extended=True, check_actions=True, task_type=task_type)
...
def run(question: str, logger: TraceLogger, workdir: Path) -> str:
    ...
    task_type = rubric.get("task_type") or "exploratory"
    session = _run_claude(PROMPT.format(date=now_str(), question=question,
                                        ...
                                        type_protocol=TYPE_PROTOCOLS.get(task_type, "")),
    ...
    quick = _verify_plus(out_file, question, workdir, deep=False, rubric=rubric_now, task_type=task_type)
    ...
    for attempt in (1, 2, 3, 4):
        rubric_now = _merged_rubric(rubric, workdir)
        result = _verify_plus(out_file, question, workdir, deep=True, rubric=rubric_now, task_type=task_type)
...
if __name__ == "__main__":
    import tempfile
    wd = Path(tempfile.mkdtemp(prefix="armF7_"))
    logger = TraceLogger("/tmp/armF7_smoke.jsonl")
